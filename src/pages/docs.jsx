...
import { Terminal, Copy, Check, Sparkles, Code2, Cpu, BookOpen, Layers } from 'lucide-react';
...
print(response.choices[0].message.content)`,
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 relative overflow-hidden selection:bg-brand-500 selection:text-white">
      ...
      <div className="absolute top-0 right-1/4 w-[500px] h-[350px] bg-brand-500/10 blur-[150px] pointer-events-none rounded-full" />
      ...
      <div className="flex relative z-10">
        ...
        <main className="flex-1 p-6 sm:p-10 space-y-10 max-w-5xl mx-auto">
          ...
          <div className="border border-slate-800/80 bg-slate-900/40 backdrop-blur-xl p-6 sm:p-7 rounded-3xl space-y-4 shadow-2xl relative">
            <div className="flex items-center justify-between">
              <h3 className="font-bold text-sm text-slate-200 flex items-center gap-2">
                <Terminal className="w-4 h-4 text-brand-400" /> Environment Variables Configuration (.env)
              </h3>
              <button
            ...
            <pre className="bg-slate-950 p-4 rounded-2xl font-mono text-xs text-brand-400 border border-slate-800/80 shadow-inner select-all">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <div className="border border-slate-800/80 bg-slate-900/40 backdrop-blur-xl p-6 rounded-3xl space-y-3 shadow-2xl">
              ...
              <p className="text-xs text-slate-400 leading-relaxed">
                All requests point directly to <code className="text-brand-400 font-mono">/v1/chat/completions</code> or model-specific proxy endpoints with standard response formatting.
              </p>
            </div>
